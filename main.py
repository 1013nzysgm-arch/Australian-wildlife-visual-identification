...
async def search_by_image(
    ...
    upload_dir = Path("backend/temp_search")
    upload_dir.mkdir(
        parents=True,
        exist_ok=True
    )
    ...
    matches = query_api.find_by_species(
        species
    )
...
class MultiSpeciesRequest(BaseModel):
    query_tags: dict
...
@app.post("/query/multiple-species")
def query_multiple_species(
    request: MultiSpeciesRequest
):

    query_api = QueryAPI()

    results = query_api.find_by_multiple_species(
        request.query_tags
    )
...
@app.get("/query/full-image")
def get_full_image(thumbnail_url: str):

    query_api = QueryAPI()

    original_url = query_api.find_by_thumbnail_url(
        thumbnail_url
    )
